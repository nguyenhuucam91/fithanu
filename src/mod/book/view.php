...
// prepare header
$pagetitle = format_text($book->name) . ": " . format_text($chapter->title);
...
echo $OUTPUT->header();
echo $OUTPUT->heading(format_text($book->name));
...
if (!$book->customtitles) {
    if (!$chapter->subchapter) {
        ...
        echo $OUTPUT->heading(format_text($currtitle), 3);
    } else {
        $currtitle = book_get_chapter_title($chapters[$chapter->id]->parent, $chapters, $book, $context);
        $currsubtitle = book_get_chapter_title($chapter->id, $chapters, $book, $context);
        echo $OUTPUT->heading(format_text($currtitle), 3);
        echo $OUTPUT->heading(format_text($currsubtitle), 4);
    }
}
